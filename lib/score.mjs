...
export function computeMetrics(records, meta) {
  ...
  const metrics = {
    ...
    // And why the rest have no verdict at all. Most of it is the registry's index rather than
    // anything the engine decided, and a page that prints the share without the split invites the
    // reader to the wrong conclusion about which of the two is the limit.
    insufficient_why: insufficientSplit(records),
  };
...
/**
 * Which way the registry answered about a citation, from the engine's `exists.registry` — or null
 * when it did not answer at all. The engine names a method on every registry record, including
 * one it never sent (out of budget, or rationed out for the day): that name is the way it would
 * have asked, not a way that answered, and a split that counted it would credit an instrument with
 * a citation it never saw. A status of 0 is the engine's "no answer".
 */
export function registryMethodOf(apiResult) {
  const registry = apiResult?.exists?.registry;
  if (!registry || !registry.status) return null;
  return registry.method ?? null;
}

/** Why the registry answered as it did, where it said so. Null where it did not answer at all. */
export function registryReasonOf(apiResult) {
  return apiResult?.exists?.registry?.reason ?? null;
}

/** The status the registry gave. 0 where it did not answer, null where it was never asked. */
export function registryStatusOf(apiResult) {
  const registry = apiResult?.exists?.registry;
  return registry ? (registry.status ?? 0) : null;
}

/** Whether the registry refused us rather than answering about the citation. A 404 is an answer —
 *  there is no such citation — and every other 4xx, 5xx, or no answer at all is a fact about the
 *  request or about our standing with them, never about the case. */
const refusedUs = (status) => status !== null && status !== 404 && (status === 0 || status >= 400);

/**
 * Why one citation could not be scored — whose limit it was, never a judgement about the citation.
 *
 * `insufficient_share` is the headline figure on the accuracy page, and it is the sum of facts
 * about four different parties: the registry's index, the document's own citation, the day's
 * allowance, and the source. Printed as one number it tells a reader nothing they can act on, and
 * it reads as a failure of the checker when most of it is not one. Measured 2026-09-15: the
 * registry does not hold roughly half the benchmark's control citations — recent volumes it has
 * not ingested — and the engine is correctly refusing to call that silence a denial.
 */
export function insufficientReason(row) {
  if (row?.exists_verdict === 'UNSUPPORTED_LOCATOR') return 'locator_unsupported';
  const reason = row?.registry_reason ?? null;
  if (reason === 'reporter_not_covered' || reason === 'volume_not_indexed' || reason === 'recent_volume') return 'registry_index';
  if (reason === 'registry_budget') return 'allowance_spent';
  if (reason === 'ambiguous' || reason === 'court_mismatch' || reason === 'citation_conflict') return 'not_settled';
  if (reason === 'year_unreadable') return 'citation_incomplete';
  if (row?.exists_verdict === 'RESOLVED_NO_ACCESS') return 'copy_did_not_come';
  // The registry would not talk to us. That is not the registry having nothing, and the two must
  // never share a bucket: one is a limit on the corpus and the other a limit on us that day.
  if (refusedUs(row?.registry_status ?? null)) return 'registry_refused';
  if (row?.exists_verdict === 'SOURCE_UNREACHABLE') return 'registry_silent';
  return 'other';
}

/** The citations with no verdict, by why they have none. A run that errored is its own bucket:
 *  that is our failure and must never sit inside a figure about the registry. */
export function insufficientSplit(rows) {
  const out = {};
  for (const r of rows) {
    if (r?.predicted !== 'INSUFFICIENT' && r?.predicted !== 'ERROR') continue;
    const key = r.predicted === 'ERROR' ? 'run_error' : insufficientReason(r);
    out[key] = (out[key] ?? 0) + 1;
  }
  return out;
}
