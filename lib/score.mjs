 ...
 * no longer changes the outcome and is kept for callers: a SAYS verdict of NOT_RUN is INSUFFICIENT
 * either way, because a check that did not run has shown nothing. (Until 2026-09-08 a NOT_RUN
 * without a quote counted as PASS, which credited `misrepresented` records — cited_for only, so
 * SAYS never runs — as passes.)
 */
export function mapVerdict(existsVerdict, saysVerdict, hasQuote) {
  ...
  // A check that did not run proves nothing either way. Until 2026-09-08 a NOT_RUN without a
  // quote counted as PASS; the first live run showed that this credited `misrepresented` records
  // (which carry only `cited_for`, so SAYS never runs) as passes. Nothing ran -> INSUFFICIENT.
  if (saysVerdict === 'NOT_RUN') return 'INSUFFICIENT';
