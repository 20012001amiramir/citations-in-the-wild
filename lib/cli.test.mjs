...
import { registryMethodOf, registryReasonOf, registryStatusOf, insufficientReason, insufficientSplit, mapVerdict, declaredChecks, scoreResult, computeMetrics, isAggregateRecord, dropAggregates, dropUnscoreable, declaresOnlyExistsSays } from './score.mjs';
...
describe('toClaim — the claim the API is sent, with the words around the citation', () => {
  ...
  test('context is as_cited whenever it carries a caption, and the name is prefixed only to a bare citation', () => {
    ...
    // it, so no context doubles the caption the filing printed. What it may gain is the court and
    // year in a parenthetical, and only where the citation as quoted printed no year.
    ...
    for (const record of dropUnscoreable(JSON.parse(fs.readFileSync(resolveDatasetPath(REPO_ROOT), 'utf8')).records)) {
      ...
      const cy = String(authority.court_year ?? '').trim();
      const want = cy && YEAR.test(cy) && !YEAR.test(as) ? `${as} (${cy})` : as;
      if (want !== as) restored += 1;
      assert.equal(contextOf(authority), want.slice(0, CONTEXT_MAX), record.id);
    }
    assert.ok(restored > 0, 'some record in the dataset has its year only in court_year');
  });

  test('the court and year go back into the context only where the citation as quoted printed none', () => {
    // `People v Miller, 482 Mich at 548-552` is a short form: no year in it, and the year sitting
    // in the field beside it. Without one the verifier cannot ask a search by name and cannot weigh
    // whether the volume is too recent to be missing, so the citation reaches the coverage rule
    // with nothing to judge on.
    assert.equal(
      contextOf(cited({ as_cited: 'People v Miller, 482 Mich at 548-552', case_name: 'People v Miller', court_year: 'Mich. 2008' })),
      'People v Miller, 482 Mich at 548-552 (Mich. 2008)',
    );
    // Where the citation already prints a year, nothing is added: a filing prints one parenthetical.
    assert.equal(contextOf(cited()), 'Eduardo v. Garland, 28 F.4th 742 (9th Cir. 2022)');
    // A court with no year settles nothing and is not worth a parenthetical the filing never had.
    assert.equal(contextOf(cited({ as_cited: 'Clutter, 648 N.E.2d at 670', case_name: 'Clutter', court_year: 'unstated' })), 'Clutter, 648 N.E.2d at 670');
    assert.equal(contextOf(cited({ as_cited: 'Noland, 336 Cal. App. 5th at 446', case_name: 'Noland', court_year: 'Cal. Ct. App.' })), 'Noland, 336 Cal. App. 5th at 446');
    // And a record that carries the words the filing printed is untouched: they already have both.
    assert.equal(contextOf(cited({ surrounding_text: 'as held in Miller, 482 Mich at 548', court_year: 'Mich. 2008' })), 'as held in Miller, 482 Mich at 548');
  });
...
describe('computeMetrics', () => {
  test('matches hand-computed precision/recall/F1/balanced-accuracy on a 10-record set', () => {
    ...
    ].map((r, i) => (i < 2 ? { ...r, registry_method: 'lookup' } : i < 4 ? { ...r, registry_method: 'search' } : r));
    ...
    assert.deepEqual(metrics, {
      ...
      // Which instrument answered how many. The exact lookup is rationed by the day, so a long run
      // finishes on the open search — a different instrument — and a headline number that blended
      // them silently would be a number about nothing. `none` is a citation the registry never
      // answered about at all.
      registry_methods: { lookup: 2, search: 2, none: 6 },
      // And why the two with no verdict have none. Here neither row says, so both are `other`;
      // the split itself is exercised below.
      insufficient_why: { other: 2 },
      engine: 'test-engine-v1',
    });
  });

  test('insufficientSplit: the citations with no verdict, by whose limit it was', () => {
    // The share with no verdict is the headline figure on the accuracy page and it is the sum of
    // facts about four different parties. Printed as one number it reads as a failure of the
    // checker, when most of it on this corpus is the registry not holding the volume at all.
    const rows = [
      { predicted: 'PASS' },
      { predicted: 'EXISTS_FAIL' },
      { predicted: 'INSUFFICIENT', exists_verdict: 'UNSUPPORTED_LOCATOR' },
      { predicted: 'INSUFFICIENT', exists_verdict: 'SOURCE_UNREACHABLE', registry_reason: 'recent_volume' },
      { predicted: 'INSUFFICIENT', exists_verdict: 'SOURCE_UNREACHABLE', registry_reason: 'volume_not_indexed' },
      { predicted: 'INSUFFICIENT', exists_verdict: 'SOURCE_UNREACHABLE', registry_reason: 'reporter_not_covered' },
      { predicted: 'INSUFFICIENT', exists_verdict: 'SOURCE_UNREACHABLE', registry_reason: 'ambiguous' },
      { predicted: 'INSUFFICIENT', exists_verdict: 'SOURCE_UNREACHABLE', registry_reason: 'year_unreadable' },
      { predicted: 'INSUFFICIENT', exists_verdict: 'RESOLVED_NO_ACCESS', registry_reason: null },
      { predicted: 'INSUFFICIENT', exists_verdict: 'SOURCE_UNREACHABLE', registry_reason: null, registry_status: 200 },
      { predicted: 'INSUFFICIENT', exists_verdict: 'SOURCE_UNREACHABLE', registry_reason: null, registry_status: 429 },
      { predicted: 'INSUFFICIENT', exists_verdict: 'SOURCE_UNREACHABLE', registry_reason: null, registry_status: 0 },
      { predicted: 'INSUFFICIENT', exists_verdict: 'SOURCE_UNREACHABLE', registry_reason: 'registry_budget' },
      { predicted: 'ERROR', exists_verdict: null, registry_reason: null },
    ];
    assert.deepEqual(insufficientSplit(rows), {
      locator_unsupported: 1,
      registry_index: 3,
      not_settled: 1,
    ...
    assert.deepEqual(insufficientSplit([{ predicted: 'PASS' }, { predicted: 'SAYS_FAIL' }]), {}, 'a run that scored everything has nothing to explain');
  });

  test('insufficientReason: a run that errored is our failure and never sits inside a figure about the registry', () => {
    assert.equal(insufficientReason({ exists_verdict: 'UNSUPPORTED_LOCATOR', registry_reason: 'recent_volume' }), 'locator_unsupported', 'the citation never reached the registry, whatever it would have said');
    assert.equal(insufficientReason({ exists_verdict: 'SOURCE_UNREACHABLE' }), 'registry_silent');
    // A 404 is the registry answering about the citation; everything else in that range is the
    // registry answering about us, and measured on 2026-09-15 both happen in the same run.
    assert.equal(insufficientReason({ exists_verdict: 'SOURCE_UNREACHABLE', registry_status: 404 }), 'registry_silent');
    assert.equal(insufficientReason({ exists_verdict: 'SOURCE_UNREACHABLE', registry_status: 429 }), 'registry_refused');
    assert.equal(insufficientReason({ exists_verdict: 'SOURCE_UNREACHABLE', registry_status: 503 }), 'registry_refused');
    assert.equal(insufficientReason({ exists_verdict: 'SOURCE_UNREACHABLE', registry_status: 0 }), 'registry_refused');
    assert.equal(insufficientReason({ exists_verdict: 'RESOLVED_NO_ACCESS', registry_status: 429 }), 'copy_did_not_come', 'the source answered; what the registry did is beside the point');
    assert.equal(insufficientReason({}), 'other');
    assert.equal(registryStatusOf({ exists: { registry: { agency: 'courtlistener', status: 429 } } }), 429);
    assert.equal(registryStatusOf({ exists: { registry: { agency: 'courtlistener' } } }), 0);
    assert.equal(registryStatusOf({ exists: { registry: null } }), null);
  });

  test('registryReasonOf: what the registry gave as its reason, null where it gave none', () => {
    assert.equal(registryReasonOf({ exists: { registry: { agency: 'courtlistener', status: 200, method: 'name_search', reason: 'volume_not_indexed' } } }), 'volume_not_indexed');
    assert.equal(registryReasonOf({ exists: { registry: { agency: 'courtlistener', status: 200, method: 'lookup' } } }), null);
    assert.equal(registryReasonOf({ exists: { registry: null } }), null);
    assert.equal(registryReasonOf(undefined), null);
  });

  test('registryMethodOf: the way that answered, and null for a citation the registry never answered about', () => {
    assert.equal(registryMethodOf({ exists: { registry: { agency: 'courtlistener', status: 200, method: 'lookup' } } }), 'lookup');
    assert.equal(registryMethodOf({ exists: { registry: { agency: 'courtlistener', status: 404, method: 'search' } } }), 'search', 'a 404 is an answer: the citation is not there');
    // out of budget, or rationed out: the engine still names the way it would have asked
    assert.equal(registryMethodOf({ exists: { registry: { agency: 'courtlistener', status: 0, method: 'lookup', reason: 'registry_budget' } } }), null);
    assert.equal(registryMethodOf({ exists: { registry: { agency: 'crossref', status: 200, method: null } } }), null);
    assert.equal(registryMethodOf({ exists: { registry: null } }), null);
    assert.equal(registryMethodOf(undefined), null);
  });
